...
export function MemoryPanel() {
  const { memories, addNotification } = useApp();
  ...
  const handleAddMemory = () => {
    ...
    addNotification({
      type: 'info',
      title: 'Bộ nhớ Tri thức',
      message: 'Tính năng thêm bản ghi trực tiếp đang được đồng bộ hóa với cơ sở dữ liệu.',
    });
  };
    ...
    <div className="flex flex-col h-full gap-3 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span style={{ ...orb, color: '#a855f7', fontSize: '11px', letterSpacing: '0.15em' }}>
            LÕI BỘ NHỚ TRI THỨC
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs">
          <div className="flex items-center gap-1">
            ...
            <span style={{ ...mono, color: 'rgba(0,245,255,0.6)', fontSize: '9px' }}>ĐÁM MÂY</span>
          </div>
          <div className="flex items-center gap-1">
            <GitBranch className="w-3 h-3" style={{ color: '#22c55e' }} />
            <span style={{ ...mono, color: 'rgba(34,197,94,0.6)', fontSize: '9px' }}>GIT</span>
          </div>
        </div>
      </div>
      ...
      >
        {[
          { icon: HardDrive, label: 'CỤC BỘ', status: 'ĐÃ ĐỒNG BỘ', color: '#22c55e', count: memories.filter(m => m.synced).length },
          { icon: Cloud, label: 'ĐÁM MÂY', status: 'HOẠT ĐỘNG', color: '#00f5ff', count: memories.filter(m => m.synced).length },
          { icon: GitBranch, label: 'GIT', status: 'ĐÃ SAO LƯU', color: '#a855f7', count: memories.filter(m => m.synced).length },
      ...
      <div className="relative flex-shrink-0">
        ...
        <input
          ...
          placeholder="Tìm kiếm bộ nhớ tri thức..."
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1.5">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {mem.synced ? (
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#22c55e' }} title="Đã đồng bộ" />
                  ) : (
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#f59e0b' }} title="Chưa đồng bộ" />
                  )}
                </div>
              </div>
              <p style={{ ...mono, color: 'rgba(255,255,255,0.45)', fontSize: '10px', lineHeight: '1.5' }} className="line-clamp-2 mb-2">
                {mem.content}
              </p>
      ...
      >
        ...
        <span style={{ ...mono, color: 'rgba(168,85,247,0.85)', fontSize: '10px' }}>THÊM BẢN GHI MỚI</span>
      </motion.button>
    </div>
  );
}
