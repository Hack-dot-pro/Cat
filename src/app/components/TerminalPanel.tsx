    ...
    <AnimatePresence>
      {terminalOpen && (
        ...
        >
          ...
          >
            {/* VS Code / Terminal Top Title Bar */}
            <div
            ...
            >
              ...
              <div className="flex items-center gap-1">
                {[
                  ...
                  { id: 'files', label: `Tệp & Dữ liệu (${files.length})`, icon: FileCode },
              ...
              <div className="flex-1 flex flex-col min-h-0 bg-[#1e1e1e]">
                ...
                >
                  ...
                  {isExecuting && (
                    <div className="flex items-center gap-2 text-cyan-400 animate-pulse font-mono">
                      ...
                      <span>Thư Ký Kim runtime đang thực thi...</span>
                    </div>
                  )}
                  <div ref={logsEndRef} />
                </div>
                ...
                >
                  ...
                  <input
                    ...
                    placeholder="Nhập lệnh (vd: python data_analysis.py, analyze sales_data.csv, npm i axios, cache)..."
            ...
            {/* TAB 3: VIRTUAL SOURCE FILES & DATASETS */}
            {activeTab === 'files' && (
              <div className="flex-1 overflow-y-auto p-5 bg-[#1e1e1e]" style={{ scrollbarWidth: 'thin', scrollbarColor: '#3c3c3c transparent' }}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 style={{ ...orb, color: '#00f5ff', fontSize: '13px', margin: 0 }}>
                      KHO MÃ NGUỒN VÀ DỮ LIỆU THƯ KÝ KIM
                    </h3>
                    <p style={{ ...mono, color: 'rgba(255,255,255,0.45)', fontSize: '11px', marginTop: 2 }}>
                      Chạy script Python (Pandas/NumPy), phân tích dữ liệu và xuất bảng tính Excel (.xlsx)
                    </p>
                  </div>
                </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {f.name.endsWith('.csv') && (
                            <button
                              onClick={() => {
                                handleRunCommand(`analyze ${f.name} download`);
                                setActiveTab('terminal');
                              }}
                              className="flex items-center gap-1.5 px-3 py-1 rounded bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 text-xs font-mono border border-cyan-500/40 cursor-pointer"
                            >
                              <span>📊 Phân tích & Xuất Excel</span>
                            </button>
                          )}
                          <button
                            onClick={() => {
                              ...
                              else if (f.name.endsWith('.csv')) handleRunCommand(`analyze ${f.name}`);
                          ...
                          >
                            <Play className="w-3 h-3 fill-current" />
                            <span>{f.name.endsWith('.py') ? 'Chạy Python' : 'Xem nội dung'}</span>
                          </button>
                        </div>
                      </div>
